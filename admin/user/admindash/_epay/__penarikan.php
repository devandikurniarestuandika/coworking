...
if(isset($_POST["kirim"])) {
    ...
    if($yangcocok == 1){
		...
		if ($kreditepay > $ikredit) {
			...
			$tambah = ($totaldeposit + $ikredit);

			$kurang = ($kreditepay - $ikredit);
			$conn->query("INSERT INTO epay_transaksi(id_epay, created_at, pesan, status_transaksi, total) VALUES('$idepay', '$idibuat', '3', '1', '$ikredit')");
			$conn->query("UPDATE epay SET kredit='$kurang' WHERE id='$idepay'");
			$conn->query("UPDATE deposit SET total='$tambah', aksi_oleh='$iaksi' WHERE id_deposit=1");
			echo "<script>window.location='index.php?halaman=epay-penarikan';</script>";
		} else {
			$alert = 2;
		}
    } else {
...
<div class="page-bar">
						<div class="page-title-breadcrumb">
							<div class=" pull-left">
								<div class="page-title">Penarikan E-Pay (Offline)</div>
							</div>
							<ol class="breadcrumb page-breadcrumb pull-right">
								...
								<li class="active">Penarikan E-Pay</li>
							</ol>
						</div>
					</div>
					<div class="tab-content tab-space">
						...
						<?php if ($alert == 1) {
								...
								?><div class="alert alert-danger">
								<strong>Galat!</strong> Kredit E-Pay tidak cukup. Minimal sisakan Rp. 1.
							</div><?php
							} ?>
											...
											<form role="form" method="post" enctype="multipart/form-data">
												...
												<div class="form-group">
													...
													<input required="" pattern=".{5,}" title="Mohon masukkan minimal setidaknya 5 karakter" maxlength="30" type="number" class="form-control" id="simpleFormEmail"
														placeholder="Masukkan Isi Kredit yang ingin ditarik" name="kredit">
												</div>
