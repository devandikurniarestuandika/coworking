...
if(isset($_POST["kirim1"])) {
	...
	$conn->query("UPDATE users SET no_ktp='$iktp', nama='$inamel', tgl_lahir='$ikelahiran', bio='$ibio', kelamin='$ikelamin', alamat='$ialamat', aksi_oleh='Diri Sendiri' WHERE id='$iidusr'");
...
if(isset($_POST["kirim2"])) {
    ...
    if(is_array($_FILES)) {
		...
		$namafile = $nama. ".". $ext;
		$conn->query("UPDATE users SET foto='$namafile', aksi_oleh='Diri Sendiri' WHERE id='$iidusr'");
		echo "<script>window.location='index.php?halaman=profil-pengguna';</script>";
    }
}
...
if(isset($_POST["kirim3"])) {
	...
	if ($ioldpass == $_SESSION['password']) {
		$conn->query("UPDATE users SET password='$inewpass', aksi_oleh='Diri Sendiri' WHERE id='$iidusr'");
		echo "<script>window.location='index.php?halaman=profil-pengguna';</script>";
	} else {
		$alert = 1;
	}
...
if(isset($_POST["kirim4"])) {
	$iidusr = $_SESSION['id'];
	$conn->query("UPDATE users SET status='0', aksi_oleh='Diri Sendiri' WHERE id='$iidusr'");
